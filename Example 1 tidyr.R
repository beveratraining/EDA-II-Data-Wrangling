#### Tutorial using the tidyr package###
...
#The Pivot_longer Function - need to pivot columns together to create a single column with many rows in a case when our data is in a wide format. 
#Reviw that the subject across multiple variables, which is not usually what we want in our tables. In order to get the data in the correct format, we will need to transform those columns into a new pair of variables.
